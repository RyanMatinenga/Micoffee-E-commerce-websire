...
<section class="orders">
  <h1 class="title">Monday Sales</h1>
  <div class="box-container">
    <table>
      ...
      <tbody>
        ...
        $select_orders = mysqli_query($conn, "SELECT * FROM `orders` WHERE DATE(order_date) = '$firstDay' ORDER BY id DESC") or die('query failed');
        $totalSalesMonday = 0;

        if (mysqli_num_rows($select_orders) > 0) {
            while ($fetch_orders = mysqli_fetch_assoc($select_orders)) {
                $user = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `users` WHERE id = '{$fetch_orders['user_id']}'"));
                $totalSalesMonday += $fetch_orders['total_price'];
                ?>
                <tr>
            ...
            echo '<tr><td colspan="7">No orders placed this Monday!</td></tr>';
        }
        ?>
        <tr>
          <td colspan="6">Total Sales for Monday:</td>
          <td>R<?= $totalSalesMonday ?></td>
        </tr>
      </tbody>
    </table>
  </div>
